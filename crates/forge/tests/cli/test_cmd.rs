...
// Tests that gas metering cheatcodes work correctly
forgetest_init!(gas_metering_all_functions, |prj, cmd| {
    prj.wipe_contracts();
...
contract GasTest is Test {
    function testPauseGasleft() public {
    ...
    function testResumeGasleft() public {
        uint256 gasStart = gasleft();
        uint256 sum = 0;
        for (uint256 i = 0; i < 100; i++) {
            sum += i;
        }
        uint256 gasUsedBefore = gasStart - gasleft();
