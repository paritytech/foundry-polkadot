...
use polkadot_sdk::{
    pallet_revive::{
        self, AccountInfo, AddressMapper, BytecodeType, ContractInfo, ExecConfig, Executable,
        Pallet, TransactionMeter, test_utils::deposit_limit,
    },
    sp_core::{self, H160},
    ...
    pub fn etch_call(
        ...
        self.0.lock().unwrap().externalities.execute_with(|| {
            ...
            let contract_blob = Pallet::<Runtime>::try_upload_code(
                ...
                code,
                code_type,
                &mut TransactionMeter::<Runtime>::new_from_limits(
                    Weight::MAX,
                    deposit_limit::<Runtime>(),
                )
                .map_err(|_| <&str as Into<Error>>::into("Could not create transaction meter"))?,
