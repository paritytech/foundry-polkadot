...
use revive_env::{AccountIdMapper, BlockAuthor, ExtBuilder, Runtime, System, Timestamp};
use std::{
    fmt::Debug,
    sync::{Arc, Mutex},
};
...
impl TestEnv {
    ...
    pub fn get_nonce(&mut self, account: Address) -> u32 {
        self.0.lock().unwrap().externalities.execute_with(|| {
            System::account_nonce(AccountIdMapper::to_fallback_account_id(&H160::from_slice(
                account.as_slice(),
            )))
        })
    }
    ...
    pub fn set_nonce(&mut self, address: Address, nonce: u64) {
        self.0.lock().unwrap().externalities.execute_with(|| {
            let account_id =
                AccountIdMapper::to_fallback_account_id(&H160::from_slice(address.as_slice()));
    ...
    pub fn etch_call(
        ...
        self.0.lock().unwrap().externalities.execute_with(|| {
            ...
            let origin_account = AccountIdMapper::to_fallback_account_id(&origin_address);

            let target_address = H160::from_slice(target.as_slice());
            let target_account = AccountIdMapper::to_fallback_account_id(&target_address);
                ...
                AccountInfo::<Runtime>::load_contract(&target_address)
            {
                ...
                System::inc_account_nonce(AccountIdMapper::to_fallback_account_id(&target_address));
                contract_info
            };
    ...
    pub fn set_block_author(&mut self, new_author: Address) {
        self.0.lock().unwrap().externalities.execute_with(|| {
            let account_id32 =
                AccountIdMapper::to_fallback_account_id(&H160::from_slice(new_author.as_slice()));
