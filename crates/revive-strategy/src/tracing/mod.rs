...
use polkadot_sdk::{
    ...
    pallet_revive::{
        evm::{
            CallTrace, CallTracer, PrestateTrace, PrestateTraceInfo, PrestateTracer, PrestateTracerConfig, Trace, Tracer, TracerType
        }, tracing::trace as trace_revive, Config, Pallet, U256
    },
};
...
pub fn trace<T: Config, R, F: FnOnce() -> R>(f: F) -> (R, Option<CallTrace<U256>>, PrestateTrace) {
    let mut call_tracer = match Pallet::<revive_env::Runtime>::evm_tracer(TracerType::CallTracer(None))
    {
