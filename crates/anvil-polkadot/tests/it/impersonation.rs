...
#[tokio::test(flavor = "multi_thread")]
async fn test_auto_impersonate_via_rpc() {
    let anvil_node_config = AnvilNodeConfig::test_config();
    let substrate_node_config = SubstrateNodeConfig::new(&anvil_node_config);
    let mut node = TestNode::new(anvil_node_config.clone(), substrate_node_config).await.unwrap();
...
#[tokio::test(flavor = "multi_thread")]
async fn test_auto_impersonate_via_cli() {
    ...
    // Auto impersonation enabled previously via cli, so transferring from a random destination
    // should work.
    let alith_balance = node.get_balance(alith_account.address(), None).await;
    let dest_balance = node.get_balance(dest_h160, None).await;
    let transfer_amount = U256::from(1e11);
